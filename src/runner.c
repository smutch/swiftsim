 ...
 * e->stars_properties->max_smoothing_iterations; */
/*   int redo = 0, scount = 0; */
...
/*           for (struct link *l = finger->stars.density; l != NULL; l =
 * l->next) { */
...
/*               runner_doself_subset_branch_stars_density(r, finger, sparts,
 * sid, */
/*                                                         scount); */
...
/*   const int max_smoothing_iter =
 * e->hydro_properties->max_smoothing_iterations; */
/*   int redo = 0, bcount = 0; */
...
/*       error("Can't allocate memory for sid."); */
/*     if ((h_0 = (float *)malloc(sizeof(float) * c->black_holes.count)) ==
 * NULL) */
/*       error("Can't allocate memory for h_0."); */
/*     if ((left = (float *)malloc(sizeof(float) * c->black_holes.count)) ==
 * NULL) */
/*       error("Can't allocate memory for left."); */
/*     if ((right = (float *)malloc(sizeof(float) * c->black_holes.count)) ==
 * NULL) */
/*       error("Can't allocate memory for right."); */
...
/*               /\* Left or right? *\/ */
/*               if (l->t->ci == finger) */
/*                 runner_dopair_subset_branch_bh_density(r, finger, bparts,
 * sid, */
/*                                                        bcount, l->t->cj); */
/*               else */
/*                 runner_dopair_subset_branch_bh_density(r, finger, bparts,
...
/*               /\* Left or right? *\/ */
/*               if (l->t->ci == finger) */
/*                 runner_dosub_subset_bh_density(r, finger, bparts, sid,
 * bcount, */
/*                                                l->t->cj, 1); */
/*               else */
/*                 runner_dosub_subset_bh_density(r, finger, bparts, sid,
...
/* #ifdef SWIFT_DEBUG_CHECKS */
/*         /\* Is this part within the timestep? *\/ */
/*         if (!part_is_active(p, e)) error("Ghost applied to inactive
 * particle"); */
/* #endif */
...
/*             if (with_cosmology) { */
/*               const integertime_t ti_step =
 * get_integer_timestep(p->time_bin); */
/*               const integertime_t ti_begin = */
/*                   get_integer_time_begin(ti_current - 1, p->time_bin); */

/*               dt_alpha = */
/*                   cosmology_get_delta_time(cosmo, ti_begin, ti_begin +
 * ti_step); */
/*             } else { */
/*               dt_alpha = get_timestep(p->time_bin, time_base); */
/*             } */
...
/*             error( */
/*                 "Smoothing length correction not going in the right
 * direction"); */
/* #endif */
...
/*             tracers_after_init(p, xp, e->internal_units,
 * e->physical_constants, */
/*                                with_cosmology, e->cosmology, */
/*                                e->hydro_properties, e->cooling_func,
 * e->time); */
 ...
 * for */
/*          * the evolution of alpha factors (i.e. those involved in the
 * artificial */
...
/*           /\* Run through this cell's density interactions. *\/ */
/*           for (struct link *l = finger->hydro.density; l != NULL; l =
 * l->next) { */
